      ...
      <table className="text-color w-full table-fixed text-left text-sm">
          ...
          {learnerAnalytics?.moduleAnalytics.map((module, index) => {
            return (
              ...
              >
                ...
                <th className="py-4 px-6 flex flex-row">{clickedIndex[index] ? <ChevronDownIcon className="w-4 h-4 text-color mr-2"></ChevronDownIcon> : <ChevronRightIcon className="w-4 h-4 text-color mr-2"></ChevronRightIcon>}<Link href={`Java/${module.name}`}>{module.name}</Link></th>
        ...
        <div className=" font-normal">12 tasks</div>
                ...
                <td className="flex flex-row py-4 px-6 ">
                  ...
                  <div className="text-xs">{module.overallProgress *100} %</div>
                </td>
